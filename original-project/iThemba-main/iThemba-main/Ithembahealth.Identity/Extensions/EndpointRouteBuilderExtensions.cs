...
using Ithembahealth.Identity.Data;
...
public static class EndpointRouteBuilderExtensions
{
        ...
        where TUser : class, new()
    {
        ...
        routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>>
            ([FromBody] ExtendedRegisterRequest registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            ...
            {
                appUser.FullName = registration.FullName;
                appUser.ClinicName = registration.ClinicName;
                appUser.MedicalLicenseNumber = registration.MedicalLicenseNumber;
            }
            ...
            if (!string.IsNullOrWhiteSpace(registration.Role))
            {
                ...
                {
                    var roleResult = await roleManager.CreateAsync(new IdentityRole(registration.Role));
                ...
                var addToRoleResult = await userManager.AddToRoleAsync(user, registration.Role);
                if (!addToRoleResult.Succeeded)
                {
                    return CreateValidationProblem(addToRoleResult);
                }
            }
    ...
    //    };
    //}
    private static async Task<ExtendedInfoResponse> CreateInfoResponseAsync<TUser>(
